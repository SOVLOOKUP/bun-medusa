...
# Edit freely — runtime-generated values (BASE_URL-derived CORS,
# secrets, HMR env, DATABASE_URL, etc.) are synced on every subsequent boot.
...
# --- (4a) BASE_URL one-line derivation -------------------------------------
# Single public origin (e.g. `https://medusa.example.com:8443`).  When the
# user sets BASE_URL we auto-derive:
#   • STORE_CORS / ADMIN_CORS / AUTH_CORS (browsers that call the backend)
#   • ADMIN_URL (admin-bundler absolute post-login redirects)
#   • HMR_HOSTNAME / HMR_PROTOCOL / HMR_CLIENT_PORT for admin-vite-overrides.ts
#
# Explicit values of the derived vars (STORE_CORS, ADMIN_URL, HMR_*, ...)
# ALWAYS win — only the UNSET ones are filled in, so the user can tweak a
# single derived var without disabling the BASE_URL shorthand entirely.
if [ -n "${BASE_URL:-}" ]; then
  BASE_URL="${BASE_URL%/}"
  case "$BASE_URL" in
    https://*) _PROTO=https; _WS_PROTO=wss ;;
    http://*)  _PROTO=http;  _WS_PROTO=ws  ;;
    *)
      echo "[dev-entrypoint] WARN: BASE_URL='$BASE_URL' missing scheme, assuming http://" >&2
      _PROTO=http; _WS_PROTO=ws
  ...
  # HMR env vars consumed by admin-vite-overrides.ts:
  #   • clientPort: "80"/"443" become "" (empty sentinel, see overrides file)
  ...
  : "${HMR_HOSTNAME:=$_HOST}"
  : "${HMR_PROTOCOL:=$_WS_PROTO}"
  case "${HMR_CLIENT_PORT+x}" in
    "") export HMR_CLIENT_PORT="$_CLIENT_PORT" ;;  # user hasn't set → fill
  esac

  export STORE_CORS ADMIN_CORS AUTH_CORS ADMIN_URL HMR_HOSTNAME HMR_PROTOCOL

  echo "[dev-entrypoint] BASE_URL=$BASE_URL  →  derived CORS + ADMIN_URL + HMR defaults."
fi

# --- (4a1) Secrets auto-generation ----------------------------------------
# Strong random hex secrets.  Priority:
#   (1) explicit container env (user's top-level .env)
#   (2) value previously persisted in $APP_DIR/.env
#   (3) fresh generate + persist
random_hex32() {
...
is_strong_value() {
  [ -n "$1" ] || return 1
  [ "${#1}" -ge 64 ] || return 1
  case "$1" in change_me*|*placeholder*|*supersecret*) return 1 ;; esac
  return 0
}
ensure_secret() {
  key="$1"; label="$2"
  eval current=\"\${${key}:-}\"
  if is_strong_value "$current"; then
    export "${key}=${current}"
    echo "[dev-entrypoint] $label ✓ explicit container env value used."
    return 0
  fi
  if [ -f "$APP_DIR/.env" ]; then
    from_file="$(sed -n "s/^${key}=//p" "$APP_DIR/.env" 2>/dev/null | tail -n1 || true)"
    if is_strong_value "$from_file"; then
      export "${key}=${from_file}"
      echo "[dev-entrypoint] $label ✓ value from $APP_DIR/.env (bind-mount) reused."
      return 0
    fi
  fi
  ...
  echo "[dev-entrypoint] $label ✨ auto-generated (length=${#generated}); persisted to $APP_DIR/.env."
}
ensure_secret JWT_SECRET                 "JWT_SECRET"
ensure_secret COOKIE_SECRET              "COOKIE_SECRET"
ensure_secret AUTH_MFA_ENCRYPTION_KEY    "AUTH_MFA_ENCRYPTION_KEY"

# --- (4a2) Sync runtime env into .env (idempotent every boot) -------------
# Keep the "user edits win" behaviour — only write keys that weren't set
# manually in the file (we use awk for DATABASE_URL below), or that we
# generated BASE_URL-derived values for.  This guarantees that even if the
# user mounted an old medusa-app/.env, the current runtime vars stay in sync.
for _KV in \
  "STORE_CORS=${STORE_CORS:-http://localhost:8000}" \
  "ADMIN_CORS=${ADMIN_CORS:-http://localhost:5173,http://localhost:9000}" \
  "AUTH_CORS=${AUTH_CORS:-http://localhost:5173,http://localhost:8000,http://localhost:9000}" \
  ...
  ensure_env_var() {
    key="${_KV%%=*}"; val="${_KV#*=}"
    _ENV_TMP="${APP_DIR}/.env.kv.$$"
    awk -v newval="${key}=${val}" '
      BEGIN { replaced = 0 }
      {
        k = "'"$key"'"
        # Compare literal prefix to avoid regex-special chars in key names
        if (!replaced && substr($0, 1, length(k)+1) == k "=") { print newval; replaced = 1 }
        else { print $0 }
      }
      END { if (!replaced) print newval }
    ' "$APP_DIR/.env" > "$_ENV_TMP" 2>/dev/null && mv "$_ENV_TMP" "$APP_DIR/.env"
    rm -f "$_ENV_TMP"
  }
  ensure_env_var
  unset -f ensure_env_var
done
# Add a final newline if missing (subsequent writes would otherwise concat).
if [ -f "$APP_DIR/.env" ] && [ "$(tail -c1 "$APP_DIR/.env" 2>/dev/null | wc -l)" -eq 0 ]; then
  echo "" >> "$APP_DIR/.env"
fi
